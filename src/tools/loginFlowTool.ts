...
export class LoginFlowTool extends BaseNavigationTool {
    ...
    async execute(args: z.infer<typeof this.inputSchema>, context: import('@/types/index.js').NavigationToolContext): Promise<import('@/types/index.js').ToolResult> {
        ...
        try {
            ...
            if (!session.hasConsentHandled) {
                try {
                    const {ConsentHandler} = await import('../core/consentHandler.js');
                    ...
                    console.warn('Cookie consent handling failed on login page', {error});
                }
            }
            ...
            return {
                content: [
                    ...
                    {
                        type: 'image',
                        data: formScreenshot.toString('base64'),
                        mimeType: 'image/png'
                    },
                    {
                        type: 'image',
                        data: resultScreenshot.toString('base64'),
                        mimeType: 'image/png'
                    }
                ]
            };
            ...
            const errorMessage = `Login flow failed: ${error instanceof Error ? error.message : String(error)}`;
            const errorData = {
                success: false,
                error: errorMessage,
                sessionId: session.id,
    ...
    private async detectUsernameField(page: import('playwright').Page): Promise<string | null> {
        const selectors = [
            'input[type="email"]',
    ...
    private async detectPasswordField(page: import('playwright').Page): Promise<string | null> {
        const selectors = [
            'input[type="password"]',
            'input[name*="pass" i]',
            'input[id*="pass" i]',
    ...
    private async detectSubmitButton(page: import('playwright').Page): Promise<string | null> {
        const selectors = [
            'button[type="submit"]',
            'input[type="submit"]',
