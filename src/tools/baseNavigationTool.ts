...
import {NavigationToolContext} from '@/types/index.js';
...
export abstract class BaseNavigationTool extends BaseTool {
    ...
    protected async getOrCreateSession(args: Record<string, unknown>, context: NavigationToolContext): Promise<PageSession> {
    ...
    protected createSessionResult(session: PageSession, additionalData: Record<string, unknown> = {}): Record<string, unknown> {
        return {
    ...
    protected async safeElementInteraction<T>(
        page: Page,
        selector: string,
        action: (element: unknown) => Promise<T>,
        ...
        try {
            ...
            throw new Error(`Failed to interact with element '${selector}': ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
