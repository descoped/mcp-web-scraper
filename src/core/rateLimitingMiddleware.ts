...
import { 
  ...
  RateLimitResult,
  RateLimitAction,
  RateLimitMiddlewareContext
} from '../types/rateLimiting.js';
import type { IStructuredLogger, IMetricsCollector } from '../types/monitoring.js';
...
export function createHttpRateLimitMiddleware(
  ...
  return async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    const context: RateLimitContext = {
      ...
      ...((req as any).connectionId && { connectionId: (req as any).connectionId }),
      timestamp: Date.now()
    };
...
export class MCPRateLimitingWrapper {
  ...
  private handleRateLimitViolation(result: RateLimitResult, context: RateLimitContext): never {
    ...
    (error as any).code = result.error?.code || 'RATE_LIMIT_EXCEEDED';
