...
import type {ITool, IToolRegistry, ToolContext, ToolResult} from '@/types/index.js';
...
export abstract class BaseTool implements ITool {
     ...
     */
    protected validateArgs<T>(args: Record<string, unknown>, schema: unknown): T {
        try {
            const parseFunction = (schema as { parse?: (args: Record<string, unknown>) => T }).parse;
            if (typeof parseFunction === 'function') {
                return parseFunction(args);
            }
            throw new Error('Schema does not have a parse function');
        } catch (error) {
            throw new McpError(
