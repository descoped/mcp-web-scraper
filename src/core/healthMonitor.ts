...
import {MetricNames} from '@/core/metricsCollector.js';
import type {IBrowserPool, IConnectionManager, IToolRegistry} from '@/types/index.js';
import type {StreamingManager} from '@/core/streamingManager.js';
...
export class HealthMonitor implements IHealthMonitor {
    ...
    private checkBrowserPoolHealth(): HealthStatus['components']['browserPool'] {
        try {
            const poolStatsRaw = (this.browserPool as unknown as {
                getStats?: () => unknown
            }).getStats?.() as Record<string, unknown> || {};

            const poolStats = {
                activeBrowsers: typeof poolStatsRaw.activeBrowsers === 'number' ? poolStatsRaw.activeBrowsers : 0,
                availableBrowsers: typeof poolStatsRaw.availableBrowsers === 'number' ? poolStatsRaw.availableBrowsers : 0,
                queuedRequests: typeof poolStatsRaw.queuedRequests === 'number' ? poolStatsRaw.queuedRequests : 0
            };
    ...
    private checkConnectionsHealth(): HealthStatus['components']['connections'] {
        try {
            const connectionStatsRaw = (this.connectionManager as unknown as {
                getStats?: () => unknown
            }).getStats?.() as Record<string, unknown> || {};

            const connectionStats = {
                totalConnections: typeof connectionStatsRaw.totalConnections === 'number' ? connectionStatsRaw.totalConnections : 0,
                oldestConnection: typeof connectionStatsRaw.oldestConnection === 'string' ? connectionStatsRaw.oldestConnection : undefined
            };
    ...
    private checkToolsHealth(): HealthStatus['components']['tools'] {
        try {
            const toolStatsRaw = (this.toolRegistry as unknown as {
                getStats?: () => unknown
            }).getStats?.() as Record<string, unknown> || {};

            const toolStats = {
                totalTools: typeof toolStatsRaw.totalTools === 'number' ? toolStatsRaw.totalTools : 0
            };
